...
def process_video(inFile, outFile, exportVid=True):
    ...
    if exportVid:
        ...
        out = cv2.VideoWriter(f'./{outFile}_mpVid.mp4', fourcc, 20, (int(cap.get(3)),int(cap.get(4))))
    with mp_pose.Pose(
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                tLndMrks = list(map(lambda lndMrk: (lndMrk.x, lndMrk.y, lndMrk.z, lndMrk.visibility,
                                                    lndMrk.presence) if lndMrk else None, results.pose_landmarks.landmark))
            else:
                tLndMrks = None
                tData = None
...
def main():
    ...
    if len(args) < 2 or args[0] == "--help":
        print("usage: python self_care_selfies.py <VIDEO_INPUT_FOLDER_PATH> <VIDEO_OUTPUT_FODLER_PATH>")
        exit()
    in_folder = args[0]
    out_folder = args[1]
