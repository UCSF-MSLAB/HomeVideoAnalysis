...
clean_hv_consent_path <- file.path("C:/Users/mmccu/Box/MM_Personal/5_Projects/BoveLab/3_Data_and_Code/gait_bw_zeno_home_analysis",
                                   analysis_version, 
                                   "004_demographics_feasibility", 
                                   "home_video_consent_first_visit_clean.csv")
...
```{r}
# participants who were approached for BW, but did not consent to home videos 
no_hv_consent_path <- file.path("C:/Users/mmccu/Box/MM_Personal/5_Projects/BoveLab/3_Data_and_Code/gait_bw_zeno_home_analysis",
                                   analysis_version, 
                                   "004_demographics_feasibility", 
                                   "home_video_no_consent_ms_first_visit_clean.csv")

no_hv_consent_df <- read.csv(no_hv_consent_path)
head(no_hv_consent_df)
```
...
clean_hv_video_path <- file.path("C:/Users/mmccu/Box/MM_Personal/5_Projects/BoveLab/3_Data_and_Code/gait_bw_zeno_home_analysis",
                                   analysis_version, 
                                   "004_demographics_feasibility", 
                                   "home_video_sent_first_visit_clean.csv")
...
factor_vars <- function(df){
  ...
  #race/ethnicity 2 - 1) white and asian, 2) blck and hispanic, 3) others 
  df <- df %>%
    mutate(race_ethnicity_minority = case_when(
      race_ethnicity_clean == "White Non Hispanic" ~ "White Non Hispanic or Asian",
      race_ethnicity_clean == "Asian" ~ "White Non Hispanic or Asian", 
      race_ethnicity_clean == 'Black Or African American' ~ 'Black or African American or Hispanic or Latino',
      race_ethnicity_clean == 'Hispanic or Latino' ~ 'Black or African American or Hispanic or Latino', 
      race_ethnicity_clean == 'Other/Unknown/Declined' ~ 'Other/Unknown/Declined'
      ))
...
no_hv_consent_df <- factor_vars(no_hv_consent_df)
clean_hv_video_df <- factor_vars(clean_hv_video_df)
```


# QC - mututally exclusive groups 
```{r}
# bw id in no consent df that are also in consent data frame - should equal zero 
no_hv_consent_df %>% 
  filter((bw_id %in% clean_hv_consent_df$bw_id))

```

# create new data frame - video vs no video 
```{r}
...
table(no_video_df$clean_Sex_2)
table(clean_hv_video_df$clean_Sex_2)

table(clean_hv_consent_df$ms_dx_condensed_2)
table(no_video_df$ms_dx_condensed_2)
table(clean_hv_video_df$ms_dx_condensed_2)
...
```
## Race and Ethnicity - original values 
...
```
## Race Ethnicity: minority vs not minority 
```{r}
# consented to home videos vs did not consent to home videos 

print('----- missing ------')
sum(is.na(no_hv_consent_df$race_ethnicity_minority))
sum(is.na(clean_hv_consent_df$race_ethnicity_minority))

# format table 
consent_race_minority_tab <- rbind(table(no_hv_consent_df$race_ethnicity_minority), table(clean_hv_consent_df$race_ethnicity_minority))
dimnames(consent_race_minority_tab)[1] <- list("Participants:" = c("Did not consent", "Consented"))

consent_race_minority_tab


consent_race_minority_xsq <- chisq.test(consent_race_minority_tab)
consent_race_minority_xsq
consent_race_xsq_row <- print_save_xsq_results(consent_race_minority_xsq, 'Race and Ethnicity - Minority')
```

```{r}
# sent home video vs did not send home video
print('----- missing ------')
sum(is.na(no_video_df$race_ethnicity_minority))
sum(is.na(clean_hv_video_df$race_ethnicity_minority))


video_race_minority_tab <- rbind(table(no_video_df$race_ethnicity_minority), table(clean_hv_video_df$race_ethnicity_minority))


dimnames(video_race_minority_tab)[1] <- list("Participants:" = c("Did not sent video", "Sent video"))
video_race_minority_tab
video_race_minority_fisher <- fisher.test(video_race_minority_tab)
video_race_minority_fisher
video_race_minority_fisher_row <- print_save_fisher_results(video_race_minority_fisher, 'Race and Ethnicity - Minority')
...
consent_cat_results <- rbind(consent_sex_xsq_row, consent_dx_xsq_row, consent_race_fisher_row,
                             consent_race_xsq_row) %>% 
  mutate(formatted_p_val = ifelse(p_value < 0.001, '<0.001', round(p_value, 3)))
consent_cat_results
```
...
video_cat_results <- rbind(video_sex_xsq_row, video_dx_fisher_row, video_race_fisher_row, 
                           video_race_minority_fisher_row) %>% 
  mutate(formatted_p_val = ifelse(p_value < 0.001, '<0.001', round(p_value, 3)))
video_cat_results
```
...
# test - shapiro says normal, but doesn't look normal.. using wilcox
video_age_wilcox <- wilcox.test(x = no_video_df$clean_Age, 
...
video_age_wilcox 
video_age_results_row <- save_test_results(video_age_wilcox, 'Age')
...
consent_summary_all <- consent_summary_all %>% 
  mutate(IQR = consent_summary_all$q3 - consent_summary_all$q1)
...
video_summary_all <- video_summary_all %>% 
  mutate(IQR = video_summary_all$q3 - video_summary_all$q1)

video_summary_all 
write.csv(video_summary_all, file.path(output_dir, 'summary_video_cont.csv'))
```
...
```{r}
consent_cat_results <- format_pvals(consent_cat_results)
video_cat_results <- format_pvals(video_cat_results)
results_consent_cont <- format_pvals(results_consent_cont)
results_video_cont <- format_pvals(results_video_cont)
```
...
## Save 
```{r}
write.csv(consent_cat_results, file.path(output_dir,
                                         'results_consent_vs_not_categorical.csv'))

write.csv(video_cat_results, file.path(output_dir,
                                       'results_video_vs_no_video_categorical.csv'))

write.csv(results_consent_cont, file.path(output_dir, 
                                          'results_consent_vs_not_cont.csv'))

write.csv(results_video_cont, file.path(output_dir, 
                                        'results_video_vs_no_video_cont.csv'))
