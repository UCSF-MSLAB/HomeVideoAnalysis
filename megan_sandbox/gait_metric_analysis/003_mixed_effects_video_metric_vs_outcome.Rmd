...
#    mutate(Variable = 'Pixel Height Proxy')
  ...
  print(formula1)
  uni1_log <- lmer(formula1, data = data, REML = FALSE)
...
#    mutate(Variable = 'Swing % Mean') 
  ...
  tidy_uni_df <- bind_rows(uni5_df, uni6_df, uni7_df, 
                           uni1_log_df, uni2_df, uni3_df, uni4_df, 
...
### Univariate Models 
```{r}
run_save_univariate(zeno_fw_clean_df, fw_t25fw_outcome_col, 'FW', plot_x_adj = 0.5)
```
...
### Univariate Models 
```{r}
run_save_univariate(home_clean_df, home_t25fw_outcome_col, 'Home', plot_x_adj = 1)
```
...
t25fw_home_multi_metrics_formula <- as.formula(paste(home_t25fw_outcome_col, "~ stride_time_median_sec_pose_hv +
                                                      ...
                                                      singlesupport_time_sec_std_pose_hv +
                                                      (1|bw_id)"))
...
### Univariate Models 
```{r}
run_save_univariate(zeno_pws_clean_df, pws_pwsVel_outcome_col, 'PWS', plot_x_adj = 55)
```
...
### Univariate Models
```{r}
run_save_univariate(zeno_fw_clean_df, fw_fwVel_outcome_col, 'FW', plot_x_adj = 50)
```
...
### Univariate Models 
```{r}
run_save_univariate(home_clean_df, home_fwVel_outcome_col, 'Home', plot_x_adj = 30)
```
...
### Univariate Models 
```{r}
run_save_univariate(home_clean_df, home_pwsVel_outcome_col, 'Home', plot_x_adj = 30)
